  ...
  <div class="event-body">
    ...
    <div class="event-footer">
      ...
      {#if hasRewards}<div class="rewards" aria-label={`Rewards: ${event.rewardLabel ?? event.rewards?.map(reward => reward.label).join(', ')}`} title={event.rewardLabel}>
        ...
        {#each event.rewards ?? [] as reward}<span class="reward-item" class:free-pulls={reward.freePulls} title={reward.label}>{#if reward.icon}<img src={reward.icon} alt="" width="26" height="26" loading="lazy" decoding="async" onerror={hideImage}/>{:else}<Icon name={reward.fallbackIcon ?? 'gift'} size={17}/>{/if}<span>{reward.amount}</span></span>{/each}
      </div>{/if}
      {#if hasPickups}<div class="pickups" aria-label="Featured pickups">{#each event.pickups ?? [] as pickup}<span class:character={pickup.kind === 'character'} title={pickup.name}><img src={pickup.image} alt={pickup.name} width={pickup.kind === 'character' ? 44 : 42} height={pickup.kind === 'character' ? 52 : 42} loading="lazy" decoding="async" onerror={hideImage}/></span>{/each}{#if event.overflowPickups}<span class="pickup-overflow">+{event.overflowPickups}</span>{/if}</div>{/if}
...
<style>
  ...
  .event-media,.event-body{position:relative;z-index:2;pointer-events:none}.event-media{display:flex;align-items:center;justify-content:center;width:100%;aspect-ratio:512/125;overflow:hidden;border-bottom:1px solid var(--border-subtle)}.event-media img{display:block;width:100%;height:100%;object-fit:contain}
  .event-body{display:flex;flex-direction:column;gap:6px;min-width:0;padding:12px}.metadata{display:flex;align-items:center;min-width:0;overflow:hidden;color:var(--event-accent);font-size:11px;line-height:16px;font-weight:600;white-space:nowrap}.kind{display:flex;align-items:center;gap:4px;min-width:0}.kind>span,.metadata-item{overflow:hidden;text-overflow:ellipsis}.metadata-item{display:flex;align-items:center;min-width:0;color:var(--text-secondary)}.metadata-item::before{content:'·';margin:0 5px;color:var(--text-muted);flex:none}.metadata-item.rerun{color:var(--accent-warning)}.metadata-item.predicted{color:var(--text-muted)}
  h3{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;line-clamp:2;min-width:0;margin:0;overflow:hidden;font-size:14px;line-height:19px;font-weight:650;color:var(--text-primary)}.schedule{display:flex;align-items:center;gap:5px;min-width:0;color:var(--event-accent);font-size:11px;line-height:16px;font-variant-numeric:tabular-nums}.schedule time{color:var(--text-secondary)}.context{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text-muted);font-size:10px}.context::before{content:'·';margin-right:5px}.details-hint{display:flex;align-items:center;gap:2px;margin-left:auto;font-size:10px;color:var(--text-muted);white-space:nowrap}
  .event-footer{display:flex;flex-wrap:wrap;align-items:center;gap:8px;position:relative;z-index:3;min-width:0;margin-top:3px;padding-top:8px;border-top:1px solid var(--border-subtle);pointer-events:none}.empty-footer{display:flex;align-items:center;justify-content:flex-end;gap:4px;width:100%;font-size:11px;color:var(--text-muted);min-height:22px}.empty-footer :global(svg){color:var(--event-accent)}
  ...
  .race-lines{display:grid;min-width:0;width:100%;order:0;gap:2px}.race-lines span{font-size:11px;line-height:16px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.race-lines span:first-child{color:var(--text-primary);font-weight:600}.has-race .rewards{flex-basis:100%;padding-top:6px;border-top:1px solid var(--border-subtle)}
  ...
  @media(pointer: coarse) and (max-width: 1300px){.plan{min-height:44px;min-width:44px}}@media(prefers-reduced-motion:reduce){.event-card{transition:none}}
</style>
