...
<article class="event-card" class:is-mobile={mobile} class:has-media={hasImage} class:no-pickups={!hasPickups} class:has-race={hasRace} class:can-plan={event.canPlan} data-event-type={event.eventType} data-event-id={event.id}>
  ...
  <div class="event-body">
    ...
    <div class="event-footer">
      ...
      {#if hasPickups}<div class="pickups" aria-label="Featured pickups">{#each event.pickups ?? [] as pickup}<a href={pickup.gametoraUrl} target="_blank" rel="noopener noreferrer" draggable="false" class:character={pickup.kind === 'character'} title={pickup.name}><img src={loadImages ? pickup.image : undefined} alt={pickup.name} width={pickup.kind === 'character' ? 44 : 42} height={pickup.kind === 'character' ? 52 : 42} loading="lazy" decoding="async" onerror={hideImage}/></a>{/each}{#if event.overflowPickups}<span class="pickup-overflow">+{event.overflowPickups}</span>{/if}</div>{/if}
...
<style>
  ...
  .event-media,.event-body{position:relative;z-index:2;pointer-events:none}.event-media{display:flex;align-items:center;justify-content:center;width:100%;aspect-ratio:512/125;overflow:hidden;background:var(--surface-2);border-bottom:1px solid var(--border-subtle)}.event-media img{display:block;width:100%;height:100%;object-fit:contain;opacity:0;transition:opacity 160ms}.event-media.loaded img{opacity:1}.media-placeholder{position:absolute;inset:0;display:grid;place-items:center;color:var(--event-accent);opacity:.3}
  ...
  .pickups{display:flex;align-items:center;gap:5px;min-width:0;order:0}.pickups>a{display:block;flex:none;width:40px;height:40px;overflow:hidden;border-radius:4px}.pickups>a.character{height:46px;width:40px}.pickups>a{pointer-events:auto;transition:background 140ms}.pickups>a:hover,.pickups>a:focus-visible{background:var(--surface-3);outline:1px solid var(--event-accent);outline-offset:1px}.pickups img{display:block;width:100%;height:100%;object-fit:contain}.pickup-overflow{font-size:11px;color:var(--text-muted)}
  ...
  .is-mobile .plan{min-height:32px}.is-mobile .metadata{font-size:9px}.is-mobile.no-pickups:not(.has-race) .event-body{min-height:93px;grid-template-rows:15px minmax(18px,auto) 17px minmax(32px,1fr)}
