<script lang="ts">
  ...
  const label = $derived(value === null ? 'Limit break not recorded' : `Limit break ${value} of 4`);
</script>
...
<style>
  .lb-diamonds { display:inline-flex; align-items:center; gap:inherit; color:var(--text-secondary); }
  svg { width:1em; height:1em; flex:none; fill:var(--text-disabled); }
  svg.filled { fill:var(--accent-primary); }
</style>
