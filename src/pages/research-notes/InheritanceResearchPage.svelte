<script lang="ts">
  ...
  import { factorMetadata, watchFactorCatalog } from '@/lib/catalog/factor-catalog';
  ...
  const rows = $derived((source?.topFactors ?? []).map((factor) => ({
    factor: factorMetadata(factor.factorId)?.text ?? `Factor ${factor.factorId}`,
    stars: `${factor.stars}★`,
  ...
  onMount(() => { void load(); return watchFactorCatalog(); });
</script>
