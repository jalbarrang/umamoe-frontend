    ...
    {#if node.layer > 0 && node.layer < 3}
      <section class="sparks" aria-label={`Sparks for ${node.label}`}>
        <header class="node-toolbar">
          ...
          <span class="race-action"><Button variant="ghost" size="sm" icon="trophy" ariaLabel={`Edit race wins for ${node.label}`} onclick={onraces}>Race wins{#if node.winSaddleIds.length} · {node.winSaddleIds.length}{/if}</Button><Button variant="ghost" size="sm" onclick={onmodechange}>{perRun ? 'Per Run' : 'Per Inh.'}</Button></span>
        </header>
...
<style>
  ...
  .composition{display:flex;align-items:center;flex-wrap:wrap;gap:5px;padding-top:10px;border-top:1px solid var(--border-subtle);font-size:11px;color:var(--text-muted)}.composition-label{flex-basis:100%;font-size:10px}.term{padding:3px 5px;border-radius:4px;background:var(--color-accent-soft);color:var(--color-accent)}.term.shared{background:rgb(129 199 132/.1);color:var(--accent-success)}.composition>strong{color:var(--accent-pink)}
   ...
   .affinity-breakdown{display:grid;grid-template-columns:auto auto auto;flex:1;min-width:220px;max-width:380px;margin:0;border:1px solid var(--border-subtle);border-radius:4px;background:var(--surface-2)}.affinity-breakdown>div{display:flex;align-items:center;justify-content:space-between;gap:6px;padding:5px 7px}.affinity-breakdown>div+div{border-left:1px solid var(--border-subtle)}.affinity-breakdown dt{display:flex;align-items:center;gap:4px;color:var(--text-muted);font-size:10px;white-space:nowrap}.affinity-breakdown dd{margin:0;font-size:12px;font-weight:650;font-variant-numeric:tabular-nums}.spark-affinity{color:var(--accent-purple)}.spark-affinity :global(svg){color:var(--accent-purple)}.base{color:var(--accent-pink)}.race{color:var(--accent-warning)}.race-action{margin-left:auto;display:flex;align-items:center;gap:6px}.race-action :global(.ui-button){min-height:26px;padding:2px 4px;font-size:10px}
