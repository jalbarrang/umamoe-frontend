<script lang="ts">
  ...
  import { cardsForType, chartAxisBounds, groupTierCards, limitBreakLabel, TIER_DEFINITIONS, tierForPercentile, type TierCard, type TierlistData } from '@/lib/tierlist/tierlist';
...
<SourcePage routeId="tierlist" title="Support Card Tierlist" width="wide">
  ...
  {:else}
    <ContentAd routeId="tierlist" top/>
    <section class="chart"><div class="card-chart" bind:clientWidth={chartWidth} style:height={`${chartHeight}px`} aria-label={`${types[Number(type)]} support card power scores`}>{#each ticks as tick}<i class="grid-line" style={`left:${4 + (tick - axis.minimum) / (axis.maximum - axis.minimum) * 92}%`}><span>{tick.toLocaleString()}</span></i>{/each}{#each chartPoints as point (point.card.id)}<button class="chart-card" style={`--x:${point.x}%;--stack:${point.stack};--tier:${point.color}`} title={`${point.card.name} · ${(point.card.scores[Number(limitBreak)] ?? 0).toLocaleString()}`} aria-haspopup="dialog" onmouseenter={(event) => preview(point.card, event)} onmouseleave={() => hovered = undefined} onfocus={(event) => preview(point.card, event)} onblur={() => hovered = undefined} onclick={() => inspect(point.card)}><img src={supportCardImagePath(point.card.id)} alt={point.card.name} loading="lazy" decoding="async"/></button>{/each}<span class="chart-axis">Power Score</span></div></section>
    ...
    <ContentAd routeId="tierlist" index={2}/>
