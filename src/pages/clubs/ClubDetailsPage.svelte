<script lang="ts">
  ...
  import { calculateMemberMetrics, clubProgression, type ClubMemberMetric } from '@/lib/clubs/member-metrics';
  import { clubPolicyLabels, clubMetrics, clubMetric, memberMetric, readClubConfig, clubRankIcon as rankIcon, clubInitialPeriod, clubDataStatus, formatClubTimeAgo } from '@/lib/clubs/club-display';
  import SourcePage from '@/layouts/SourcePage.svelte';
  import Banner from '@/components/Banner.svelte';
  import Button from '@/components/Button.svelte';
  import DiscordText from '@/components/DiscordText.svelte';
    ...
    <div class="content">
      ...
      {:else if details}
        ...
        <section class="members-section" aria-label="Club members">
          {#if listMode === 'grid'}<div class="members-grid" use:virtualScroll={{ items: members, searchText: member => [member.name, member.viewerId, member.role, member.fanCount, member.fanCount.toLocaleString()].join(' '), key: member => member.viewerId, estimate: 230, onrange: range => virtualRange = range }}>{#each members.slice(virtualRange.start,virtualRange.end) as member, localIndex (member.viewerId)}{@const index = virtualRange.start + localIndex}<article data-virtual-index={index} class="member-card" class:inactive={!member.active} style:--placement-color={member.active ? ['#ffd700', '#90a4ae', '#cd7f32'][index] : undefined}>
            ...
            <dl class="member-stats">{#each extraMetrics.filter((metric) => metric.key !== 'fanCount') as metric}<div><dt title={metric.label}>{metric.label}</dt><dd class:positive={member[metric.key] > 0} class:negative={member[metric.key] < 0}>{signed(member[metric.key])}</dd></div>{/each}{#if config.showLastUpdated}<div><dt>Last Updated</dt><dd title={member.lastUpdated}>{updated(member.lastUpdated)}</dd></div>{/if}</dl>
          </article>{/each}</div>
          {:else}<div class="member-table-scroll"><table>
            ...
            <tbody use:virtualScroll={{ items: members, searchText: member => [member.name, member.viewerId, member.role, member.fanCount, member.fanCount.toLocaleString()].join(' '), key: member => member.viewerId, estimate: 45, onrange: range => virtualRange = range }}>{#each members.slice(virtualRange.start,virtualRange.end) as member, localIndex (member.viewerId)}{@const index = virtualRange.start + localIndex}
              <tr data-virtual-index={index} class:inactive={!member.active}>
...
<style>
  ...
  .member-stats { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:4px 16px; padding-top:6px; border-top:1px solid var(--border-subtle); }.member-stats>div { display:flex; align-items:baseline; justify-content:space-between; gap:4px; min-width:0; }.member-stats dt { min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:var(--text-muted); font-size:10px; }.member-stats dd { flex:none; margin-left:auto; max-width:100%; font-size:11px; font-weight:600; line-height:1.4; font-variant-numeric:tabular-nums; white-space:nowrap; }
