      ...
      {:else if details}
        ...
        <section class="members-section" aria-label="Club members">
          ...
          {:else}<div class="member-table-scroll"><table>
            ...
            <tbody use:virtualScroll={{ items: members, key: member => member.viewerId, estimate: 45, onrange: range => virtualRange = range }}>{#each members.slice(virtualRange.start,virtualRange.end) as member, localIndex (member.viewerId)}{@const index = virtualRange.start + localIndex}
