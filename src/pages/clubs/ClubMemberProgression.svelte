...
<style>
  ...
  @media(pointer: coarse) and (max-width: 1300px),(max-width:767px) { .chart-legend button { min-height:var(--touch-target); }.calendar-day :global(.trigger) { min-height:var(--touch-target); min-width:var(--touch-target); }.popover-header { min-height:var(--touch-target); padding-right:48px; } }
