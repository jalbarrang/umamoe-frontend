<script lang="ts">
  ...
  onMount(() => { void loadCharacterCatalog().then(data => characters = data).catch(() => catalogError = 'Character names could not be loaded. Reload to retry.'); });
  ...
  import HakuUploadZone from '@/components/hakuraku/HakuUploadZone.svelte';
  import HakuReplayControls from '@/components/hakuraku/HakuReplayControls.svelte';
  ...
  const runnerOptions = $derived((capture?.runners ?? []).map((entry) => ({ value: String(entry.index), label: `${runnerName(entry, characters)} · ${placement(entry.result?.finishOrder)}` })));
  const replayRunners = $derived((capture?.runners ?? []).map((entry, index) => ({
    id: String(entry.index),
    index: entry.index,
    name: runnerName(entry, characters),
    image: runnerImage(entry, characters),
  ...
  const comparisonRows = $derived((capture?.runners ?? []).slice().sort((a, b) => (a.result?.finishOrder ?? 999) - (b.result?.finishOrder ?? 999)).map((entry) => ({
    ...
    runner: runnerName(entry, characters),
  ...
  $effect(() => {
    ...
    void Promise.all([import('./race-skill-display'), loadSkillCatalog()]).then(([{ activatedSkills }, skills]) => { if (token === skillLoadToken) skillRows = activatedSkills(current, index, skills); }).catch(() => { if (token === skillLoadToken) catalogError = 'Skill names could not be loaded. Reload to retry.'; });
  });
</script>
...
<SourcePage routeId="race-analysis" title="Race Analysis" width="wide" source="hakuraku">
  {#if catalogError}<Banner title="Catalog unavailable" tone="warning"><p>{catalogError}</p></Banner>{/if}
  ...
  {:else}
    ...
    {#if runner}
        <section class="runner-heading">
          <Artwork src={runnerImage(runner, characters)} alt={runnerName(runner, characters)} shape="circle"/>
          <div><span>{runner.isPlayer ? 'Your runner' : 'Race runner'}</span><h2>{runnerName(runner, characters)}</h2><p>{runner.trainerName ?? 'Trainer unavailable'} · {runningStyleName(runner.result?.runningStyle)}</p></div>
        ...
        <StatStrip items={stats} label={`${runnerName(runner, characters)} stats`}/>
