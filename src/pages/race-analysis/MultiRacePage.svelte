<script lang="ts">
  ...
  let skills = $state.raw(new Map<number, SkillCatalogEntry>());
  let catalogError = $state('');
  onMount(() => { void loadSkillCatalog().then(data => skills = data).catch(() => catalogError = 'Skill names could not be loaded. Reload to retry.'); });
  ...
  import { hakuBarChartOption } from '@/components/charts/haku-chart-options';
  ...
  import { formatPercentage, runningStyleName } from '@/lib/race/race-display';
