import type { PlannerRewardEntry, PlannerRewardResource } from './carat-planner';
import { plannerRewardBundles } from './planner-reward-currencies';
import { itemIconPath } from '@/lib/catalog/item-icons';
import { plannerTimestampDateKey as rewardDateKey } from './planner-calendar';
...
export function withTimelineRewardFallbacks(
  ...
  const fallbackRewards = timelineEvents.flatMap(event => {
    if (!event.id || event.eventType !== 'story_event' || eventsWithCarats.has(event.id)) return [];
    const availableAt = rewardDateKey(
      event.estimatedEndDate ?? event.date ?? event.jpReleaseDate,
    );
...
function addStandardCompetitiveFallbacks(
  ...
  for (const event of timelineEvents) {
    ...
    if (event.eventType === 'champions_meeting') {
    ...
    if (event.eventType !== 'legend_race') continue;
