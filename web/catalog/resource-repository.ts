...
export const resourceRepository = {
  ...
  async load<T>(name: string, refresh = false, decode: (data: unknown) => T = data => data as T, unpublished?: () => T): Promise<T> {
    return cache.get(`resource:${name}`, 24 * 60 * 60 * 1000, async () => {
      const request = Symbol(name);
      requests.set(name, request);
      const manifestResponse = await fetch(`${baseUrl}/manifest.json`, { cache: 'no-cache', credentials: 'omit' });
      ...
      const response = await fetch(url, { credentials: 'omit', cache: refresh ? 'reload' : 'default' });
