import factorData from '../../src/data/factors.json';
import factorIconMappingData from '../../src/data/factor_icon_mapping.json';
import { skillImage } from './skill-catalog';
...
const metadata = new SvelteMap((factorData as FactorMetadata[]).map((entry) => [Number(entry.id), entry]));
...
/** Angular's bundled fallback → persisted cache → live resource, shared by every factor consumer. */
export function loadFactorCatalog(refresh = false): Promise<void> {
  ...
  loading = (async () => {
    if (!loaded && !get(factorCatalogState).cached) {
      ...
      } catch { /* Invalid legacy caches must not replace the bundled fallback. */ }
    }
    try {
      let published = true;
      const factors = await resourceRepository.load('factors', refresh, normalizeFactors, () => { published = false; return factorOptions(); });
      if (published) replaceFactors(factors);
      loaded = true; retryAttempt = 0;
      factorCatalogState.set({ loading: false, cached: !published && get(factorCatalogState).cached, error: '' });
    } catch (reason) {
...
export function loadFactorArtwork(): Promise<void> {
  return artworkLoading ??= loadRaceFactorImages().then((images) => {
...
/** Angular profile exports use encoded factor_id values; retain older known base-ID imports too. */
export function decodeFactorEntry(factorId: number, level?: number): DecodedFactor {
  const decoded = decodeFactor(factorId);
  return decoded.type === -1 && metadata.has(factorId) && level !== undefined
...
export function factorImage(id: number): string | undefined {
  ...
  return skillImage((factorIconMappingData as Record<string, string>)[String(id)])
