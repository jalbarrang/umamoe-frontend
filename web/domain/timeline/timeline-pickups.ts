import { characterImagePath, type CharacterNames } from '../../catalog/character-catalog';
import { supportCardImagePath, type SupportCardCatalogEntry } from '../../catalog/support-card-catalog';
import type { TimelineRecord } from '../../features/timeline/timeline-repository';
import type { TimelinePickup } from '../../ui/timeline-types';
...
export function timelinePickup(id: number, catalog: TimelinePickupCatalog, label?: string, kind: 'character' | 'support' = id >= 100000 ? 'character' : 'support'): TimelinePickup {
  if (kind === 'support') {
    ...
    return { id: String(id), kind, image: supportCardImagePath(id), name: publicName(label, card?.name ?? `Support ${id}`), subLabel: [[null, 'R', 'SR', 'SSR'][card?.rarity ?? 0], type ? `${type} Support` : 'Support card'].filter(Boolean).join(' · '), searchTerms: ['support', ...(type ? [type] : [])] };
  }
  ...
  return { id: String(id), kind, name: variant ? `${base} [${variant}]` : base, subLabel: variant ? `${variant} variant` : 'Character', image: characterImagePath(id), searchTerms: [base, ...(variant ? [variant, `${base} ${variant}`] : [])] };
}
