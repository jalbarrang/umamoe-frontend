...
<style>
  ...
  .lineage-tree { --lineage-line: #5a6470; min-width: 0; display: flex; flex-direction: column; padding: 12px; border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); background: var(--surface-1); }
  .root { width: min(100%, 620px); align-self: center; }
  ...
  .branches { min-width: 0; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
  .branch { position: relative; min-width: 0; display: flex; flex-direction: column; }
  ...
  .grandparents { min-width: 0; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 6px; }
  ...
  @container lineage (max-width: 700px) {
    .lineage-tree { padding: 7px; }
    .root { width: 100%; }
    .root-fork { height: 12px; }
    .root-fork::before { left: 17px; height: 12px; transform: none; }
    .root-fork::after { display: none; }
    .branches { position: relative; grid-template-columns: 1fr; gap: 10px; margin-left: 16px; padding-left: 13px; border-left: 2px solid var(--lineage-line); }
    .branch::before { top: 24px; left: -13px; width: 13px; height: 2px; transform: none; }
    .branch:last-child::after { position: absolute; top: 26px; bottom: 0; left: -15px; width: 4px; background: var(--surface-1); content: ''; }
    .branch-fork { height: 9px; }
    .branch-fork::before { left: 16px; height: 9px; transform: none; }
    .branch-fork::after { display: none; }
    .grandparents { grid-template-columns: 1fr; gap: 5px; margin-left: 16px; padding-left: 12px; border-left: 2px solid var(--lineage-line); }
    .legacy::before { top: 20px; left: -12px; width: 12px; height: 2px; transform: none; }
    .legacy:last-child::after { position: absolute; top: 22px; bottom: 0; left: -14px; width: 4px; background: var(--surface-1); content: ''; }
  }
</style>
