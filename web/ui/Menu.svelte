<script lang="ts">
  ...
  export interface MenuItem { id: string; label: string; icon?: IconName; danger?: boolean; disabled?: boolean; checked?: boolean; separator?: boolean; }
  interface Props { label: string; ariaLabel?: string; menuLabel?: string; icon?: IconName; iconOnly?: boolean; items: MenuItem[]; onselect?: (id: string) => void; }
  let { label, ariaLabel, menuLabel, icon, iconOnly = false, items, onselect }: Props = $props();
  const id = $props.id();
  ...
  function buttons() { return [...panel.querySelectorAll<HTMLButtonElement>('button:not(:disabled)')]; }
  ...
  function keydown(event: KeyboardEvent) {
    ...
    const enabled = buttons(), index = enabled.indexOf(document.activeElement as HTMLButtonElement);
...
<div class="menu">
  <button bind:this={control} type="button" class="trigger" class:icon-only={iconOnly} aria-label={ariaLabel ?? label} title={iconOnly ? label : undefined} aria-haspopup="menu" aria-expanded={open} aria-controls={id} popovertarget={id} onkeydown={event => { if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); panel.showPopover(); focusItem(event.key === 'ArrowUp'); } }}>
    {#if icon}<Icon name={icon} size={17}/>{/if}{#if !iconOnly}<span>{label}</span><Icon name="chevron" size={15}/>{/if}
  </button>
  <div bind:this={panel} {id} class="menu-panel" popover="auto" role="menu" tabindex="-1" aria-label={menuLabel ?? ariaLabel ?? label} style:left="{left}px" style:top="{top}px" ontoggle={toggle} onkeydown={keydown}>
    {#each items as item}
      ...
      <button type="button" role={item.checked === undefined ? 'menuitem' : 'menuitemradio'} aria-checked={item.checked} class:active={item.checked} class:danger={item.danger} disabled={item.disabled} onclick={() => select(item.id)}>
...
<style>
  ...
  .menu-panel button { min-height:36px; display:flex; align-items:center; gap:var(--space-2); padding:4px 12px; border:0; border-radius:var(--radius-sm); background:transparent; color:var(--factor-option-text); cursor:pointer; font-size:var(--font-sm); text-align:left; }
  .menu-panel button>span{overflow-wrap:anywhere}.menu-panel button:hover:not(:disabled),.menu-panel button:focus-visible{background:var(--factor-option-hover)}
  ...
  @media (max-width: 768px), (pointer:coarse) { .menu-panel button,.trigger { min-height:var(--touch-target); }.trigger.icon-only{min-width:var(--touch-target)} }
</style>
