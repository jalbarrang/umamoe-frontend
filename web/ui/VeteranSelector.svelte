<script lang="ts">
  ...
  export interface VeteranOption { id: string; name: string; rank: string; detail: string; image?: string; }
...
<label class="selector" for={id}>
  ...
  <span class="control"><Artwork src={selected?.image} alt={selected?.name ?? 'No Veteran selected'} size="sm"/><span class="copy"><strong>{selected?.name ?? 'Select a Veteran'}</strong><small>{selected ? `${selected.rank} · ${selected.detail}` : 'Local workspace'}</small></span><select {id} bind:value {disabled} aria-label={label}>{#each options as option}<option value={option.id}>{option.name} — {option.rank}</option>{/each}</select></span>
</label>

<style>
  .selector { display: grid; gap: 6px; }
  .label { font-size: var(--font-sm); font-weight: 600; }
  .control { position: relative; min-height: 58px; display: flex; align-items: center; gap: var(--space-3); padding: 8px 42px 8px 8px; border: 1px solid var(--factor-field-border); border-radius: var(--radius-md); background: var(--factor-field-bg); }
