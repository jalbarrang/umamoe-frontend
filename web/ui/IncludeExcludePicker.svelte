...
<style>
  ...
  @media(pointer:coarse){.filter-row,.compact .filter-row,.chip button{min-height:var(--touch-target)}.chip button{min-width:var(--touch-target)}.filter-row.icon-only{flex-basis:44px;width:var(--touch-target);min-height:var(--touch-target)}}
</style>
