...
<style>
  ...
  article { min-width: 0; display: grid; grid-template-columns: minmax(0, 1fr) auto; overflow: hidden; border: 1px solid var(--border-subtle); border-radius: var(--radius-md); background: var(--surface-1); transition: background var(--duration-fast), border-color var(--duration-fast), transform var(--duration-fast), box-shadow var(--duration-fast); }
  ...
  .veteran-select { min-width: 0; display: grid; grid-template-columns: auto minmax(0, 1fr); align-items: center; gap: 9px; padding: 7px 9px; border: 0; background: transparent; color: var(--color-text); cursor: pointer; text-align: left; }
  .record { min-width: 0; display: grid; gap: 5px; }
  header { min-width: 0; display: flex; flex-wrap: wrap; align-items: center; gap: 5px; }
  header > strong { max-width: 220px; overflow: hidden; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
  .scenario { padding: 1px 5px; border: 0; border-radius: var(--radius-xs); background: rgb(var(--accent-secondary-rgb) / .1); color: var(--accent-secondary); font-size: 8px; }
  header :global(.rank--sm) { width: 24px; height: 24px; }
  .detail { color: var(--color-text-subtle); font-size: 9px; }
  .sparks { min-width: 0; display: flex; flex-wrap: wrap; gap: 3px; }
  .parent-preview { min-width: 0; display: grid; gap: 2px; padding-top: 3px; border-top: 1px solid var(--border-subtle); }
  ...
  .actions { display: flex; align-items: center; flex-direction: column; justify-content: center; gap: 2px; padding: 4px; border-left: 1px solid var(--border-subtle); }

  @container (max-width: 520px) {
    article:hover { transform: none; }
    .veteran-select { align-items: start; gap: 5px; padding: 4px; }
    .veteran-select :global(.art--portrait.art--md) { width: 32px; height: 44px; border-width: 1px; }
    .record { gap: 3px; }
    header { gap: 3px; }
    ...
    .sparks { gap: 2px; }
    .sparks :global(.spark:nth-child(n + 9)) { display: none; }
