<script lang="ts">
  ...
  interface Props { id: string; title: string; description?: string; legend?: ChartLegendItem[]; state?: 'ready' | 'loading' | 'empty' | 'error'; emptyTitle?: string; emptyDetail?: string; children?: Snippet; onexport?: () => void; }
  let { id, title, description, legend = [], state = 'ready', emptyTitle = 'No data to chart', emptyDetail = 'Adjust the filters or choose another comparison.', children, onexport }: Props = $props();
</script>

<figure class="chart-frame" aria-labelledby={`${id}-title`}>
  <figcaption><div><h3 id={`${id}-title`}>{title}</h3>{#if description}<p>{description}</p>{/if}</div>{#if onexport}<button type="button" onclick={onexport}><Icon name="download" size={15}/>Export</button>{/if}</figcaption>
...
<style>
  .chart-frame { min-width: 0; margin: 0; display: grid; gap: 8px; padding: 10px; border: 1px solid var(--border-primary); border-radius: var(--radius-lg); background: var(--surface-1); container: chart-frame / inline-size; }
  ...
  .legend { display: flex; flex-wrap: wrap; gap: 5px 12px; margin: 0; padding: 6px 0; border-block: 1px solid var(--border-subtle); list-style: none; }.legend li { display: inline-flex; align-items: center; gap: 5px; color: var(--color-text-muted); font-size: 9px; }.legend li > span { width: 8px; height: 8px; border-radius: 2px; background: var(--series); }
  .plot { min-width: 0; min-height: 180px; overflow: hidden; }.state { min-height: 180px; display: grid; place-items: center; align-content: center; gap: 5px; padding: 12px; border: 1px dashed var(--border-primary); border-radius: var(--radius-sm); color: var(--color-text-subtle); text-align: center; }.state strong { color: var(--color-text); font-size: var(--font-sm); }.state span { max-width: 40ch; font-size: 10px; }
