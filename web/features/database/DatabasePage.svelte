<script lang="ts">
  ...
  import { tourStepId, completeTourInteraction } from '../../platform/tours/tour-state';
  import SourcePage from '../../ui/layout/SourcePage.svelte';
  import Banner from '../../ui/Banner.svelte';
  import Button from '../../ui/Button.svelte';
  import Dialog from '../../ui/Dialog.svelte';
  import EmptyState from '../../ui/EmptyState.svelte';
  import FilterPresetMenu, { type FilterPreset } from '../../ui/FilterPresetMenu.svelte';
  import FilterShell from '../../ui/FilterShell.svelte';
  import Pagination from '../../ui/Pagination.svelte';
  import Icon from '../../ui/Icon.svelte';
  import SelectField from '../../ui/SelectField.svelte';
  import ScenarioMultiSelect from '../../ui/ScenarioMultiSelect.svelte';
  ...
  import { authUser, authReady } from '../../platform/auth/auth-state';
  import { HttpError } from '../../platform/http/http-client';
  import { DISCORD_SUPPORT_URL } from '../../platform/site-links';
  import { factorOptions, loadFactorArtwork, watchFactorCatalog, factorCatalogState } from '../../catalog/factor-catalog';
  import ResourceStatus from '../../ui/ResourceStatus.svelte';
  import { characterImagePath, loadReleasedCharacterCatalog, type CharacterCatalogEntry } from '../../catalog/character-catalog';
  import { loadLiveSupportCards, readCachedSupportCards, supportCardImagePath, type SupportCardCatalogEntry } from '../../catalog/support-card-catalog';
  import { releasedSupportCards, supportCardDisplay, supportTypeName } from '../../domain/supports/support-card';
  import type { SupportCardPickerOption } from '../../ui/picker-types';
  import { accountParent, parentCharacter, type SelectableParent } from '../../domain/veterans/parent-picker';
  import { normalizeVeteranRecord } from '../../domain/veterans/veteran-normalizer';
  import { authRepository } from '../../platform/auth/auth-repository';
  import { profileRepository } from '../profile/profile-repository';
  import { activeInheritanceFilterCount, emptyInheritanceFilters, inheritanceRequestFilters, type FactorRequirement, type InheritanceFilterMode, type InheritanceSearchFilters, type InheritanceSearchResult } from '../../domain/inheritance/inheritance-search';
  import { validateInheritanceUql, type UqlValidation } from '../../domain/inheritance/uql';
  import { UqlCompiler, type UqlQueryCatalog } from '../../domain/inheritance/uql-compiler';
  import { parseUqlContext, resolveUqlTarget, resolveUqlLegacy, uqlLegacyHints, setUqlLegacy, type UqlContextIssue } from '../../domain/inheritance/uql-context';
  import ParentPickerDialog from '../parent-picker/ParentPickerDialog.svelte';
  import { deviceVeteranParents } from '../veterans/veteran-library';
  ...
  import { veteranToUi } from '../veterans/veteran-adapter';
  ...
  const veteranView = $derived(selectedParent && partner ? veteranToUi({ ...partner, name: '' }, selectedParent.trainer_id ?? '') : undefined);
  ...
  async function loadUqlCatalog(): Promise<void> {
    ...
    uqlCatalogError = '';
    try { uqlCatalog = await (await import('../../catalog/uql-catalog')).loadUqlQueryCatalog(); }
...
<SourcePage routeId="database" title="Database" width="wide">
  <div class="inheritance-database"><PageHeading eyebrow="Inheritance" title="Database" description="Find optimal inheritance pairings and support cards for Uma Musume." flush>{#snippet actions()}<Button variant="secondary" size="sm" icon="add" onclick={() => submitOpen = true}>Add Trainer ID</Button>{/snippet}</PageHeading><div class="content-container">
          ...
          <div class="context-filters" class:advanced={filterMode === 'advanced'}>
            <div class="affinity-context">
              <DatabaseFilterGroup id="affinity" bind:open={affinityOpen} title="Affinity" count={[filters.playerCharaId, selectedVeteranId || sharedLegacyLabel].filter(Boolean).length}>
                <DatabaseAffinityFilter characters={characterOptions} loading={charactersLoading} error={charactersError} onretry={loadCharacters} targetId={String(filters.playerCharaId ?? '')} veteran={veteranView} {sharedLegacyLabel} selectedAccountId={selectedParent?.trainer_id} ontargetchange={selectTarget} onveteranchange={selectVeteran}/>
              </DatabaseFilterGroup>
            </div>
    ...
    <section class="inheritance-results">
      ...
      {:else if inheritance.records.length}<div class="inheritance-list">{#each inheritance.records as record (record.id)}<InheritanceResultCard {record} activeFilters={filterMode === 'uql' ? undefined : filters} {characters} {supports} {defaultFocus} {splitSparks} {sparkPortraits} {hiddenSparkFactorIds} {affinityEngine} {raceGroups} {partner} targetId={filters.playerCharaId} bind:sparkPerRun bind:showOccurrences bind:showP2Sparks bind:collapsedWhiteSections partnerWinSaddles={filters.p2WinSaddle} bookmarked={bookmarkedIds.has(record.accountId)} actionBusy={bookmarkBusyIds.includes(record.accountId)} oncopy={copyTrainer} onshare={shareRecord} onreport={reportRecord} onbookmark={toggleBookmark} onplanner={openInPlanner} onvisible={queueBorrowView}/>{/each}</div>{#if listMode === 'paginated' && inheritance.totalPages > 1}<Pagination bind:page pages={inheritance.totalPages} onchange={() => window.scrollTo({ top: 0, behavior: 'smooth' })}/>{/if}
...
.support-lb-inline{min-width:0;flex:1;min-height:96px;display:grid;grid-template-columns:auto minmax(150px,1fr);align-items:stretch;gap:14px}.lb-control{align-self:center;min-width:0;display:grid;gap:6px}.lb-control :global(.ui-button){width:max-content}.inheritance-results{min-width:0;display:grid;gap:var(--space-3)}.inheritance-list{min-width:0;display:grid;gap:7px}.result-error{min-width:0}.result-error :global(.empty){border-color:rgb(var(--accent-error-rgb)/.35);background:rgb(var(--accent-error-rgb)/.06)}.result-error :global(.icon){color:var(--accent-error)}.result-error :global(p){color:var(--text-secondary)}.error-actions{display:flex;align-items:center;justify-content:center;gap:7px;flex-wrap:wrap}.sorting-notice{min-height:36px;display:flex;align-items:center;gap:7px;padding:5px 9px;border-left:3px solid var(--accent-primary);background:rgb(var(--accent-primary-rgb)/.08);color:var(--text-secondary);font-size:10px}.sorting-notice :global(svg){color:var(--accent-primary)}
  ...
  .infinite-sentinel { height: 1px; }.bookmark-results{min-width:0;display:grid;gap:10px}.bookmark-toolbar{min-width:0;display:grid;grid-template-columns:auto minmax(280px,1fr) auto;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border-subtle)}.bookmark-toolbar h2,.bookmark-toolbar p{margin:0}.bookmark-toolbar h2{font-size:var(--font-lg)}.bookmark-toolbar p{margin-top:2px;color:var(--text-secondary);font-size:9px}.bookmark-toolbar p span{color:var(--accent-primary)}.bookmark-filters{display:flex;align-items:center;justify-content:center;gap:3px}.bookmark-filters button{min-height:36px;display:flex;align-items:center;gap:6px;padding:0 10px;border:1px solid var(--factor-field-border);background:var(--factor-field-bg);color:var(--factor-field-text);cursor:pointer;font-size:10px;font-weight:700}.bookmark-filters button.active{border-color:rgb(var(--accent-primary-rgb)/.6);background:rgb(var(--accent-primary-rgb)/.12);color:var(--accent-primary)}.bookmark-filters small{min-width:17px;padding:1px 4px;background:var(--surface-3);font-size:8px}.bookmark-actions{display:flex;align-items:center;justify-content:flex-end;gap:5px}.trainer-submit{display:grid;gap:16px}.trainer-submit p{margin:0;color:var(--text-secondary);font-size:13px;line-height:1.4}.submission-field{position:relative}.submission-count{position:absolute;top:0;right:0;color:var(--text-secondary);font-size:11px;line-height:1.4;font-variant-numeric:tabular-nums}.submission-count.complete{color:var(--accent-success)}.trainer-submit :global(.field-label){padding-right:52px}.trainer-submit :global(input){font-size:16px;font-variant-numeric:tabular-nums;letter-spacing:.5px}.trainer-submit :global(.field-message){font-size:12px;color:var(--text-secondary)}.trainer-submit :global(.field-error){color:var(--color-danger)}
  ...
  @media (max-width:940px){.bookmark-toolbar{grid-template-columns:1fr}.bookmark-filters{justify-content:flex-start}.bookmark-actions{justify-content:flex-start}}
  ...
  @media (max-width:600px) { .content-container{padding:1rem 4px}.search-quick>.field-grid{grid-template-columns:1fr}.spark-builder-help{height:auto;min-height:32px;padding-block:5px}.active-filters{padding:10px}.active-filters-inner{grid-template-columns:minmax(0,1fr) auto;align-items:center}.active-filters-inner>strong{grid-column:1}.active-filter-chips{grid-column:1/-1}.clear-current{grid-column:2;grid-row:1}.active-filter-chips button{min-height:32px;padding-inline:8px;font-size:11px}.database-tabs :global([role='tab']){padding-inline:8px}.bookmark-limit{display:none}.results-controls{display:grid;grid-template-columns:1fr 1fr}.focus-control{grid-column:1/-1}.focus-options button{min-height:var(--touch-target);flex:1;padding-inline:3px;font-size:9px}.sort-control{width:auto;grid-column:1/-1}.bookmark-filters{display:grid;grid-template-columns:repeat(3,1fr)}.bookmark-filters button{min-width:0;min-height:var(--touch-target);justify-content:center;padding-inline:3px}.bookmark-actions{display:grid;grid-template-columns:1fr 1fr}.bookmark-actions :global(.ui-button){width:100%;padding-inline:4px} }
  .legacy-quick{gap:0}
