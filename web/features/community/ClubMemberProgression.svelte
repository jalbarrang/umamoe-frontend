<script lang="ts">
  import { buildClubCalendar, buildMemberProgression, formatClubGain, type ClubCalendarDay, type ClubChartMode } from '../../domain/clubs/club-progression';
  import type { ClubMemberSnapshot } from '../../domain/clubs/member-metrics';
...
<style>
  ...
  @media(pointer:coarse),(max-width:767px) { .chart-legend button { min-height:var(--touch-target); }.calendar-day :global(.trigger) { min-height:var(--touch-target); min-width:var(--touch-target); }.popover-header { min-height:var(--touch-target); padding-right:48px; } }
