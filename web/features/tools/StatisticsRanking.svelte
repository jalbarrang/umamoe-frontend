<script lang="ts">
  import type { ChartDatum } from '../../domain/statistics/statistics';
...
<style>
  ...
  @media(pointer:coarse){.sort select,.ranking-row{min-height:var(--touch-target)}.ranking-info{min-width:32px;min-height:32px}.ranking :global(.chart-actions>button){min-height:var(--touch-target)}}
</style>
