<script lang="ts">
  ...
  import { watchFactorCatalog } from '../../catalog/factor-catalog';
  onMount(watchFactorCatalog);
  import { characterImagePath, loadCharacterCatalog, loadReleasedCharacterCatalog, type CharacterCatalogEntry } from '../../catalog/character-catalog';
  import {
  ...
  import { parentCharacter } from '../../domain/veterans/parent-picker';
  import ParentPickerDialog from '../parent-picker/ParentPickerDialog.svelte';
  import RaceWinPickerDialog from '../parent-picker/RaceWinPickerDialog.svelte';
  import RaceResultsDialog from '../database/RaceResultsDialog.svelte';
  import OptimalRacesDialog from '../database/OptimalRacesDialog.svelte';
  ...
  import type { ProfileVeteran, SuccessionChara } from '../profile/profile-repository';
  import { loadG1SaddleGroups, loadOptimalRaceRecommendations, type OptimalRaceRecommendation } from '../../catalog/race-catalog';
  ...
  import { loadSkillCatalog, skillImage } from '../../catalog/skill-catalog';
  ...
  function hydrate(input: Record<PlannerPosition, PlannerNode>): Record<PlannerPosition, PlannerNode> {
    return Object.fromEntries(BTREE_ORDER.map((position) => {
      ...
      return [position, { ...node, characterId: resolved?.[0] ?? node.characterId, name: entry?.name ?? node.name, image: characterImage(entry) ?? node.image }];
  ...
  onMount(async () => {
    ...
    try {
      const [catalog, affinityData, groups] = await Promise.all([
        loadCharacterCatalog(), veteranAffinityRepository.load().catch(() => undefined), loadG1SaddleGroups()
      ]);
      ...
      if (!payload && from !== 'db' && from !== 'profile') {
        ...
        if (cards) {
          ...
          ids.slice(0, BTREE_ORDER.length).forEach((id, index) => {
            ...
            if (position && entry) initial[position].characterId = entry[0];
          });
...
<style>
  ...
  .planner-content{display:grid;gap:14px}.tree-canvas{display:grid;gap:0;--flow-line:var(--text-primary)}
  ...
  .parents{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.parent-branch{min-width:0;display:grid;grid-template-rows:subgrid;grid-row:span 3;gap:12px}
  ...
  .grandparents{position:relative;min-width:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;align-items:start;padding-top:48px;margin-top:-12px}.grandparents::before{content:'';position:absolute;top:0;left:50%;height:16px;border-left:1px solid var(--flow-line)}.grandparents::after{content:'';position:absolute;top:16px;left:calc(25% - 2.5px);right:calc(25% - 2.5px);border-top:1px solid var(--flow-line)}.gp-branch{position:relative;min-width:0;display:grid;gap:6px}.gp-branch::before{content:'';position:absolute;top:-32px;left:50%;height:32px;border-left:1px solid var(--flow-line)}.gp-flow{position:absolute;left:50%;top:-35px;transform:translateX(-50%);background:var(--bg-primary);padding:2px 5px;font-size:10px}
  ...
  @media(max-width:1199px){.grandparents{grid-template-columns:1fr;padding:24px 0 0 36px;gap:12px}.grandparents::before{left:0;top:0;bottom:0;height:auto}.grandparents::after{left:0;right:50%;top:0}.gp-branch::before{left:-36px;top:28px;width:36px;height:0;border-left:0;border-top:1px solid var(--flow-line)}.gp-flow{left:-19px;top:18px;gap:2px;padding:2px;font-size:9px}.gp-flow>span{display:none}}
  @media(max-width:767px){.parents{grid-template-columns:1fr}.target-row{gap:6px}.planner-guide{padding:4px 2px 8px}.branch-heading::before{display:none}.branch-affinity{display:flex}.parent-connections{height:60px}.parent-connections::before{height:100%}.parent-connections::after,.parent-flow::before{display:none}.parent-flow{align-items:center;padding:0}.shared-flow{top:18px}.flow-value{padding:3px 6px}.planner-shell{padding-inline:8px}.great-toggle{min-height:44px}.inheritance-trigger{min-height:44px}.inheritance-header{min-height:44px;padding-right:48px}}
  ...
  @media(pointer:coarse),(max-width:767px){.save-row :global(input),.save-row :global(.ui-button){min-height:var(--touch-target)}.save-item :global(.icon-button){width:var(--touch-target);height:var(--touch-target)}}
</style>
