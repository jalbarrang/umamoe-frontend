<script lang="ts">
  import { plannerNodeAffinity, plannerSkillSparks, plannerSparkChance, type PlannerAffinity, type PlannerNode, type PlannerPosition, type PlannerSpark } from '../../domain/lineage/planner';
  import { sparkMetrics } from '../../domain/inheritance/spark-probability';
...
<section class="odds-panel" aria-labelledby="spark-odds-title">
  ...
  {#if !sources.length}<p class="empty">Add at least one parent to see spark inheritance odds</p>{:else}
    ...
    {#if tab === 'base'}
      <div class="odds-scroll desktop-base"><table class="odds-table" aria-label="Base spark inheritance odds"><thead>
        <tr class="base-row"><th>Base</th>{#each groups as group}{#each [1,2,3] as level}<th class:group-start={level === 1} data-tone={group.tone} aria-label={`${group.label} ${level} star base chance`}><small>{level === 1 ? group.label : ''}</small>{chance(group.type, level, 0, false)}%<span>{level}★</span></th>{/each}{/each}</tr>
      ...
      <div class="odds-scroll mobile-base"><table aria-label="Base spark inheritance odds by character"><thead><tr><th>★</th>{#each sources as source}<th title={`${source.node.name} (${source.label}) ♥ ${source.affinity}`}>{#if source.node.image}<img src={source.node.image} alt={`${source.node.name} (${source.label}) ♥ ${source.affinity}`}/>{:else}{source.label}{/if}</th>{/each}</tr></thead><tbody>{#each groups as group}{#each [1,2,3] as level}<tr data-tone={group.tone} class:group-end={level === 3}><th>{#if level === 1}<small>{group.label}</small>{/if}{level}★</th>{#each sources as source}<td>{chance(group.type, level, source.affinity).toFixed(2)}%</td>{/each}</tr>{/each}{/each}</tbody></table></div>
    {:else if tab === 'sources'}
...
<style>
  ...
  .odds-scroll{overflow:auto;max-width:100%}.odds-table{width:100%;min-width:960px;border-spacing:0;font-size:.7rem;table-layout:fixed}.odds-table th,.odds-table td{text-align:center;padding:5px 3px;border-right:1px solid var(--border-subtle);border-bottom:1px solid var(--border-subtle)}.odds-table th:first-child{width:208px}.odds-table thead{background:var(--surface-2)}.odds-table [data-tone]{color:var(--spark-color);background:color-mix(in srgb,var(--spark-color) 5%,transparent)}.odds-table tbody td{font-weight:700}.odds-source{display:flex;align-items:center;gap:10px;padding:8px 9px;text-align:left}.odds-source img{width:34px;height:34px;border-radius:50%;object-fit:cover}.odds-source span{min-width:0;display:grid;gap:3px}.odds-source b{overflow:hidden;white-space:nowrap;text-overflow:ellipsis;font-size:var(--font-sm)}.odds-source small{color:#e91e63;font-size:var(--font-xs)}.mobile-base{display:none}
  ...
  .odds-table .base-row th{height:60px;font-size:.78rem;font-weight:700;line-height:1.1;color:var(--text-primary);background:var(--surface-1)}.odds-table .base-row th:first-child{font-size:.68rem;text-transform:uppercase;letter-spacing:.08em}.base-row small{display:block;min-height:10px;margin-bottom:5px;text-transform:uppercase;font-size:.48rem;letter-spacing:.08em;text-align:left;color:var(--spark-color)}.base-row span{display:block;margin-top:6px;font-size:.54rem;color:var(--text-disabled)}.base-row .group-start,.odds-table tbody td:nth-child(3n+2){border-left:3px solid var(--spark-color)}.odds-table tbody td{height:44px;font-size:.78rem}.odds-table tr>:nth-child(3n+4){border-right:2px solid var(--border-primary)}.odds-table tbody th{padding:0}.summary-row[data-tone='blue']{background:rgb(33 150 243/.06)}.summary-row[data-tone='pink']{background:rgb(240 98 146/.05)}.summary-row[data-tone='green']{background:rgb(129 199 132/.05)}
  ...
  @media(max-width:640px){
    header{padding:8px}.desktop-base{display:none}.mobile-base{display:block}.mobile-base table{width:100%;border-spacing:0;font-size:.72rem}.mobile-base th,.mobile-base td{box-sizing:border-box;height:var(--touch-target);min-width:56px;padding:4px;text-align:center;border-right:1px solid var(--border-subtle);border-bottom:1px solid var(--border-subtle);color:var(--spark-color,var(--text-muted));font-weight:700}.mobile-base th:first-child{position:sticky;left:0;background:var(--surface-2);z-index:1;border-left:3px solid var(--spark-color,transparent);width:56px}.mobile-base th small{display:block;font-size:.44rem;text-transform:uppercase}.mobile-base img{display:block;margin:auto;width:38px;height:38px;object-fit:cover;border-radius:50%}.mobile-base .group-end>*{border-bottom:2px solid var(--border-primary)}
