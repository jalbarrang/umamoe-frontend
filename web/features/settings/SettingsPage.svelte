<script lang="ts">
  ...
  import { loadCharacterImageCatalog } from '../../catalog/character-catalog';
  import { authRepository } from '../../platform/auth/auth-repository';
  import { authReady, authUser } from '../../platform/auth/auth-state';
  import type { ApiKey, AuthIdentity, LinkedAccount } from '../../platform/auth/auth-types';
...
<style>
  ...
  @media (pointer: coarse) { .form-row input { min-height:var(--touch-target); }.token-display button { width: var(--touch-target); height: var(--touch-target); flex-basis: 44px; } }
</style>
