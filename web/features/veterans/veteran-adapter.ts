...
import { factorMetadata } from '../../catalog/factor-catalog';
import { getRankInfoFromScore } from '../../domain/rank';
import type { VeteranRecord } from '../../domain/veterans/generated/veteran-record';
import type { VeteranUiRecord, VeteranSparkGroup } from '../../ui/veteran-ui-types';
import type { SparkTone, SparkSource } from '../../ui/SparkItem.svelte';

interface CharacterEntry { id: string; name: string; image: string; }
const characters = new Map((characterData as CharacterEntry[]).map((entry) => [Number(entry.id), entry]));
...
function image(cardId: number): string | undefined { return characters.has(cardId) ? `/game-assets/character_thumbs/chara_stand_${Math.floor(cardId / 100)}_${cardId}.webp` : `/game-assets/umamusume_icons/chr_icon_${Math.floor(cardId / 100)}.webp`; }
...
export function veteranToUi(record: VeteranRecord, workspace: string): VeteranUiRecord {
  ...
  ], combinedSparks: sparkGroups(record.recordId, combineVeteranFactors([...record.factors, ...record.parents.slice(0,2).flatMap(parent => parent.factors)])), sparks: sparkGroups(record.recordId, record.factors, 'main'), parents: record.parents.slice(0, 2).map((parent, index) => ({ id: `${record.recordId}:parent:${index}`, position: index === 0 ? 'P1' : 'P2', name: characters.get(parent.cardId)?.name ?? `Character ${parent.cardId}`, image: image(parent.cardId), affinity: 0, sparks: sparkGroups(`${record.recordId}:parent:${index}`, parent.factors, 'parent') })) };
}
