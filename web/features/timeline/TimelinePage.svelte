<script lang="ts">
  ...
  import { activePlan, enabledPlannerTargets, setTimelineEvent, TIMELINE_PREFERENCES_KEY } from '../../domain/timeline/carat-planner';
  import { plannerCollection, plannerSaveError, savePlannerCollection } from './planner-state';
  import TimelineBoard from './TimelineBoard.svelte';
  import Icon from '../../ui/Icon.svelte';
  import IconButton from '../../ui/IconButton.svelte';
  import { buildTimelineLanes, timelineEndDate, type TimelineAnniversary } from '../../domain/timeline/timeline-layout';
  ...
  import type { TimelineEventData } from '../../ui/timeline-types';
  import { timelineRepository, type TimelineRecord } from './timeline-repository';
  import Dialog from '../../ui/Dialog.svelte';
  import { plannerResourceRepository } from './planner-resource-repository';
  import type { PlannerGachaEntry, PlannerRewardResource } from '../../domain/timeline/carat-planner';
  import type { TimelineCalculation } from '../../domain/timeline/timeline-prediction-types';
  import { timelineDisplayTitle, type TimelinePickupCatalog } from '../../domain/timeline/timeline-pickups';
  import { buildTimelineRewardSummaries } from '../../domain/timeline/timeline-reward-summary';
  import { timelineCardContext, timelineCardRaceLines } from '../../domain/timeline/timeline-race-facts';
  ...
  function preloadPlanner(): void {
    void import('./CaratPlanner.svelte').catch(() => {});
...
<AppPage routeId="timeline" title={tab === 'carat-planner' ? 'Carat Planner' : 'Timeline'} description={tab === 'carat-planner' ? 'Plan Carats, tickets, sparks, and banner targets.' : 'Global content releases, pickups, and events.'} metadata={tab === 'timeline' ? `${filtered.length} / ${events.length} events` : undefined} tone="brand" width="wide" flush fullBleed={tab === 'timeline'} adsEnabled={tab !== 'timeline'} fill={!mobile && tab === 'timeline'} mobileHeading="actions-only">
  ...
  {#if plannerVisited || tab === 'carat-planner'}
    <div hidden={tab !== 'carat-planner'}>
      {#await import('./CaratPlanner.svelte')}
  ...
  {#if timelineVisited || tab === 'timeline'}
    ...
    {#if error}<div class="error-toast" role="alert"><strong>Timeline resources failed to load</strong><span>{error.includes('500') ? '500 Internal Server Error' : error}</span></div>{/if}
    {#if loading}<div class="loading"><Spinner label="Loading Timeline data" size={30}/><span>Loading timeline…</span></div>
    ...
    {:else}
      <TimelineBoard bind:this={timelineBoard} active={tab === 'timeline'} lanes={dateLanes} events={filtered} {anniversaries} end={endDate} {now} {mobile} {view} compact={compactGaps}>
        {#snippet card(event: TimelineRecord)}<div id={`timeline-event-${event.id}`}><TimelineEventCard event={cardView(event)} {mobile} planned={plannedIds.includes(event.id)} onplan={plan} onopen={openDetails}/></div>{/snippet}
      </TimelineBoard>
    {/if}
    </section>
  {/if}
...
<style>
  ...
  .error-toast{position:fixed;z-index:100;top:calc(var(--utility-height) + 62px);left:50%;width:min(290px,calc(100vw - 16px));display:grid;gap:2px;padding:10px 14px;border:1px solid var(--accent-error);border-radius:5px;background:var(--surface-overlay);color:var(--accent-error);transform:translateX(-50%);font-size:12px}.error-toast span{font-size:11px}.loading{min-height:320px;display:flex;align-items:center;justify-content:center;gap:12px;color:var(--text-secondary)}
