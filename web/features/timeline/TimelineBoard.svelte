<script lang="ts">
  ...
  import { buildTimelineFeed, LANE_STEP, LANE_WIDTH, timelineDateKey, timelineMonths, timelinePosition, type TimelineAnniversary, type TimelineLane, type TimelineMarker } from '../../domain/timeline/timeline-layout';
  ...
  function measureViewport() {
    if (!active) return;
    if (board) viewportWidth = board.clientWidth;
  ...
  onMount(() => {
    initialized = true;
    return () => { cancelAnimationFrame(frame); cancelAnimationFrame(momentum); };
  });
</script>
...
<style>
  ...
  @media(pointer:coarse){.lane-overflow :global(.ui-button){min-height:var(--touch-target)}}@media(max-width:360px){.mobile-feed{padding-right:8px}}
</style>
