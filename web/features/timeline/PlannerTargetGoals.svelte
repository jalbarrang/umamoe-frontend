<script lang="ts">
  import { findGacha, plannerPickupGoals, type PlannerDataBundle, type PlannerGoalProjection, type PlannerTarget, type TargetProjection } from '../../domain/timeline/carat-planner';
  import { plannerPickupOptions } from '../../domain/timeline/planner-presentation';
  import { calculatePullDistribution, pullOutcomeSegments } from '../../domain/timeline/planner-pull-probability';
...
<style>
  ...
  .pickup-art{flex:none;display:grid;place-items:center;overflow:hidden;border-radius:var(--radius-sm);background:var(--surface-2)}.pickup-art img,.pickup-art :global(svg){grid-area:1/1}.pickup-art img{object-fit:contain;z-index:1;max-width:100%}.pickup-art img:not([hidden])~:global(svg){display:none}
  ...
  .pool-stack,.outcome-stack{display:flex;height:9px;overflow:hidden;border-radius:3px;background:var(--surface-3)}.outcome-stack{height:18px}.pool-stack>span,.outcome-stack>span{height:100%}
