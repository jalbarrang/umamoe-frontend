<script lang="ts">
  ...
  import { loadSkillCatalog, skillPointTotal, skillImage, skillRarity, type SkillCatalogEntry } from '../../catalog/skill-catalog';
  import { aptitudeGrade, totalStats } from '../../domain/profile/profile-display';
  import {
  ...
  } from '../../domain/profile/profile-veterans';
  import AptitudeGrid, { type AptitudeItem } from '../../ui/AptitudeGrid.svelte';
  ...
  import { getRankInfoFromScore, getRankInfoFromLabel } from '../../domain/rank';
  ...
  import { loadG1SaddleGroups, loadRaceQueryValues, type RaceQueryValue } from '../../catalog/race-catalog';
  ...
  import { profileVeteranAffinity } from '../../domain/profile/profile-veteran-metrics';
  import { compileVeteranQuery, veteranQueryRow } from '../../domain/profile/profile-veteran-query';
  ...
  import { loadCatalog, type CatalogEntry } from '../database/catalog-repository';
  ...
  let UqlEditor=$state<typeof import('../database/DatabaseUqlEditor.svelte').default>();
  ...
  async function loadQueryEditor() {
    ...
    if(UqlEditor) return;
    uqlEditorError='';
    try {const editor=await import('../database/DatabaseUqlEditor.svelte');if(live)UqlEditor=editor.default;}
  ...
  $effect(() => { void loadSkillCatalog().then((catalog)=>skillCatalog=catalog).catch(()=>catalogError='Skills could not be loaded. Reload the page to try again.'); });
      ...
      <div class="table-wrap" role="region" aria-label="Veteran comparison" tabindex="0">
        <table>
          ...
          <thead><tr><th scope="col">Veteran</th>
            ...
            <th scope="col" title={target ? 'Affinity for '+target.name : 'Veteran and parent affinity'} aria-sort={sortField==='affinity' ? sortDirection==='asc' ? 'ascending' : 'descending' : 'none'}><Button variant="ghost" size="sm" onclick={()=>sortTable('affinity')}>Affinity {sortField==='affinity' ? sortDirection==='asc' ? '↑' : '↓' : ''}</Button></th><th scope="col">Aptitudes</th><th scope="col">Combined sparks</th><th scope="col"><span class="sr-only">Details</span></th>
          </tr></thead>
...
<style>
  ...
  @media(pointer:coarse),(max-width:767px) { .primary-filters legend>button { min-height:var(--touch-target); min-width:var(--touch-target); }.filter-choices button { min-height:var(--touch-target); min-width:var(--touch-target); } }
  ...
  @media(pointer:coarse),(max-width:767px) { .aptitude-choice :global(.select-panel button),.aptitude-choice :global(.select-panel button:first-child) { min-height:var(--touch-target); } }
  ...
  .veteran-grid { min-width:0; display:grid; grid-template-columns:repeat(var(--grid-columns),minmax(0,1fr)); align-items:start; gap:12px; }
