<script lang="ts">
  ...
  import { tourAudience } from '../tours/tour-state';
  import { router, pendingRoute } from '../router/router';
  import { navigationForPath, routeDefinitionForPath } from '../router/route-manifest';
  import { theme, toggleTheme } from '../theme';
  import { authUser } from '../auth/auth-state';
  ...
  import MoeFooter from '../../ui/layout/MoeFooter.svelte';
  import Spinner from '../../ui/Spinner.svelte';
  import ServiceNotices from './ServiceNotices.svelte';
  import { startAnalytics, trackPageView } from '../analytics';
  ...
  let PageTour = $state<typeof import('../tours/PageTour.svelte').default>();
  ...
  function loadTour(): Promise<void> {
    ...
    return tourLoading ??= import('../tours/PageTour.svelte').then(module => { PageTour = module.default; tourLoadError = false; }).catch(() => { tourLoadError = true; }).finally(() => { tourLoading = undefined; });
  }
  ...
  $effect(() => {
    ...
    let active = true;
    void tick().then(() => import('../seo')).then(module => { if (active) module.applyRouteMetadata(path); }).catch(() => {});
    return () => { active = false; };
  });
</script>
