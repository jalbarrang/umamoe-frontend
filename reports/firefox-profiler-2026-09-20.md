...
- Five stress workflows also pass their functional checks at 8× without profiling, with **15 of 54 native interaction groups over 300 ms**. Budget enforcement was disabled to collect the entire run; the budget did not pass. UQL, timeline direction changes, race dialogs, pickers and statistics still appear among the failures. No browser-level speedup is claimed from unmatched single-run timings.
