...
test('Step-ups can be planned from the timeline, budget paid steps and survive reload', async ({ page }, info) => {
  ...
  await expect(target.getByLabel('Step-up goal odds')).toHaveText('5.8% chance');
  await target.getByLabel('Desired copies of one chosen card', { exact: true }).fill('2');
  await expect(target.getByLabel('Step-up goal odds')).toHaveText('0.2% chance');
...
test('ordinary paid banners can be added, show paid shortfalls and respect their draw limit', async ({ page }, info) => {
  ...
  await expect(target.locator('.funding')).toContainText(/0\s*\/\s*10 funded/);
  await expect(target.locator('.funding')).toContainText('1 paid Carats short');
  await expect(target.getByLabel('Paid banner', { exact: true })).toBeVisible();
  ...
  await page.evaluate(() => {
    ...
    collection.plans[0].balances.paidJewels = 1500;
    localStorage.setItem('carat-planner-plans-v1', JSON.stringify(collection));
  });
  await page.reload();
  await expect(target.locator('.funding')).toContainText('0 paid Carats short');
